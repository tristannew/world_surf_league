import scrapy
# from data_request import popup_data_request
import pandas as pd
...
def popup_data_request(url:str, heat_id:str, rnd_id:str):
    ...
    wave_range = re.findall(wave_range_pattern, response.text)

    wind_pattern = r'<div class=\\"label\\">Wind conditions<\\/div>\\n                <div class=\\"value\\">(.*?)<'
    wind = re.findall(wind_pattern, response.text)
    ...
    heat_uid = heat_uid.replace(" ", "").lower()
    ...
    scores_info = []
    for surfer_name, heat_total in heat_totals.items():
        row = {
        "heat_id": heat_uid,
...
class SurfingspiderV1Spider(scrapy.Spider):
    name = "surfingspider_v1"
    allowed_domains = ["worldsurfleague.com"]
    start_urls = [
        "https://www.worldsurfleague.com/events/2023/ct?all=1",
        "https://www.worldsurfleague.com/events/2022/ct?all=1",
        "https://www.worldsurfleague.com/events/2021/ct?all=1",
        "https://www.worldsurfleague.com/events/2020/ct?all=1",
        "https://www.worldsurfleague.com/events/2019/ct?all=1",
        "https://www.worldsurfleague.com/events/2018/ct?all=1",
        "https://www.worldsurfleague.com/events/2017/ct?all=1",
        "https://www.worldsurfleague.com/events/2016/ct?all=1",
        "https://www.worldsurfleague.com/events/2015/ct?all=1",
        "https://www.worldsurfleague.com/events/2014/ct?all=1",
        "https://www.worldsurfleague.com/events/2013/ct?all=1",
        "https://www.worldsurfleague.com/events/2012/ct?all=1",
        "https://www.worldsurfleague.com/events/2011/ct?all=1",
        "https://www.worldsurfleague.com/events/2010/ct?all=1",
        # "https://www.worldsurfleague.com/events/2009/ct?all=1",
        # "https://www.worldsurfleague.com/events/2008/ct?all=1",
        ]

    def parse(self, response):
        events_urls = response.xpath('//table//a[contains(@class, "event-schedule-details__event-name")]/@href')
        for link in events_urls: #css to href to their profile:
            yield response.follow(link.get(), callback=self.parse_events_page)
    
    def parse_events_page(self, response):
        results_urls = response.xpath('//a[text()="Results"]/@href')
        for link in results_urls:
            yield response.follow("https://www.worldsurfleague.com"+link.get(), callback=self.parse_results) 

    # NOTE: at this point, I need to parse the results for each GENDER, ROUND, HEAT
    def parse_genders(self, response):
        rounds_urls = response.xpath('//span[@class="round-name"]/parent::a/@href')
        gender_urls = [url for url in rounds_urls if url.get().find("statEventId") >= 0]
        for link in gender_urls:
            yield response.follow("https://www.worldsurfleague.com"+link.get(), callback=self.parse_results)
                
    def parse_results(self, response):
        # TODO: the a comes immediatly before the span. the span is it's child
        rounds_urls = response.xpath('//span[@class="round-name"]/parent::a/@href')
        rounds_urls = [url for url in rounds_urls if url.get().find("statEventId") == -1]
        for link in rounds_urls:
            yield response.follow("https://www.worldsurfleague.com"+link.get(), callback=self.parse_heats)

# NOTE: this curl leads somewhere messy but useful
# curl "https://www.worldsurfleague.com/events/2023/ct/66/billabong-pro-pipeline/results?heatId=92399&rnd=1710407694914&pbv=f6553ddd62cd7f8bc717283057c80be775f2c719-7183&requestName=postEventWatchHeatDetailsPopup" --compressed -H "User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:124.0) Gecko/20100101 Firefox/124.0" -H "Accept: application/json, text/javascript, */*; q=0.01" -H "Accept-Language: en-GB,en;q=0.5" -H "Accept-Encoding: gzip, deflate, br" -H "Ajax-Request: postEventWatchHeatDetailsPopup" -H "X-Requested-With: XMLHttpRequest" -H "Connection: keep-alive" -H "Referer: https://www.worldsurfleague.com/events/2023/ct/66/billabong-pro-pipeline/results" -H "Cookie: locale=en_GB; wslui={^%^22rv^%^22:3^%^2C^%^22pv^%^22:1^%^2C^%^22cpvts^%^22:1710407647485^%^2C^%^22cc^%^22:^%^22GB^%^22}; _ga=GA1.2.496146763.1710407533; _gid=GA1.2.1640446163.1710407533; usprivacy=1---; _awl=2.1710407648.5-63161fb9d7c9f1eb9a55d095423419dc-6763652d6575726f70652d7765737431-0; OptanonConsent=isGpcEnabled=0&datestamp=Thu+Mar+14+2024+09^%^3A14^%^3A46+GMT^%^2B0000+(Greenwich+Mean+Time)&version=202310.1.0&browserGpcFlag=0&isIABGlobal=false&hosts=&consentId=36639d75-4f6d-4941-851d-d226ea5758ec&interactionCount=2&landingPath=NotLandingPage&groups=C0002^%^3A1^%^2CC0004^%^3A1^%^2CC0001^%^3A1^%^2CC0005^%^3A1^%^2CC0003^%^3A1^%^2CV2STACK42^%^3A1&AwaitingReconsent=false; OneTrustWPCCPAGoogleOptOut=false; _gat=1; OptanonAlertBoxClosed=2024-03-14T09:14:46.676Z; eupubconsent-v2=CP7dmwAP7dmwAAcABBENArEsAP_gAEPgACiQg1NX_H__bW9r8Xr3aft0eY1P99j77sQxBhfJE-4FzLvW_JwXx2ExNA36tqIKmRIEu3bBIQFlHJDUTVigaogVryDMakWcgTNKJ6BkiFMRM2dYCF5vmQtj-QKY5vp9d3dx2D-t_dv83dzyz8VHn3e5_2e0eJCdA58tDfv9bROb-9IPd_58v4v0_F_rk2_eT1l_tevp7B8uft87_XU-9_-.f_wACHwAAAAA; _ga_ZE6TW33RHV=GS1.1.1710407686.1.0.1710407686.0.0.0; seerses=e; seerid=4712879d-73d7-4dc7-85ae-be1382d462b1; _dc_gtm_UA-47826941-1=1; _gat_UA-47826941-21=1; __gads=ID=c031455346e7b188:T=1710407689:RT=1710407689:S=ALNI_MZ7x62fs53Qh0Kew7O7OyhalL35vQ; __gpi=UID=00000d453d31d380:T=1710407689:RT=1710407689:S=ALNI_Mal3NEo_7XVtfZ96zlHgofYdfaUjQ; __eoi=ID=aa4f735d2790c3ed:T=1710407689:RT=1710407689:S=AA-AfjagM0pxlBj1k2kDpOuH_Cdg; _fbp=fb.1.1710407689523.619937120" -H "Sec-Fetch-Dest: empty" -H "Sec-Fetch-Mode: cors" -H "Sec-Fetch-Site: same-origin" -H "TE: trailers"
    def parse_heats(self, response):
        ######## NOW you need to scrape the heat id and round id and pass it to the data request function.
        ###### It doesn't have everything, but the things that it doesn't have are,
        ###### the date and time of the heat and some surfer info, but you can get that from the profiles,
        ###### and then join on the surfers name, or something more specific since there are 2 Nat Youngs.
        heat_hrefs = response.css("a.hot-heat__action-link:contains('Details')::attr(href)")
        heat_ids = [href.get()[href.get().find("heatId=")+7:href.get().find("&")] for href in heat_hrefs]
        round_id = response.url[response.url.find("roundId=")+8:]
        heats = []
        scores = []
        for heat_id in heat_ids:
            heat_info, scores_info = popup_data_request(url=response.url[:response.url.find("results")+len("results")],
                                                        heat_id=heat_id, 
                                                        rnd_id=round_id)
            heats.append(heat_info)
            scores += scores_info
        heats_df = pd.DataFrame(heats)
        scores_df = pd.DataFrame(scores)
        print(heats_df)
        print(scores_df)
        heats_df.to_csv(f"heats_df_{heats_df["heat_id"][0]}.csv")
        scores_df.to_csv(f"scores_df_{scores_df["heat_id"][0]+scores_df["surfer_id"][0]}.csv")
        yield heats_df

#### TODO: need to go through womens heats as well.
#### TODO: need to output scores as well.